...
class ReciprocralRankFusion:
    ...
    def reciprocal_rank_fusion_ranking(self, rank1: Dict[str,int], rank2: Dict[str,int], filter_docs: Optional[int] = None) -> List[str]:
        ...
        for key in rank1:
            combined_score_uid_info[key] = self.reciprocal_rank_fusion_formula(rank1[key], rank2.get(key, max_document_rank1), self.rrf_constant) # use max rank as len of rank1 list to remove unfair advantage in long lists

        for key in rank2:
            combined_score_uid_info[key] = self.reciprocal_rank_fusion_formula(rank2[key], rank1.get(key, max_document_rank2), self.rrf_constant) # use max rank as len of rank2 list to remove unfair advantage in long lists
    ...
    def reciprocal_rank_fusion(self, rank1: List[Dict[str,int]], rank2: List[Dict[str,int]], rrf_constant: Optional[int] = 60, filter_docs: Optional[int] = None) -> List[Document]:
        texts = self.reciprocal_rank_fusion_ranking(rank1, rank2, rrf_constant, filter_docs)
